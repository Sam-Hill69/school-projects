...
let deck = [];
//create the shuffle() function with 2 parameteres named a and b
//return the value of 0.5 minus a randomly-generated number created with the Math.random() methoddocument.getElementById("deal").onclick = function () {
function shuffle(a, b) {
  return 0.5 - Math.random();
}
function newDeck() {
  //split the contents of the deckStr variable at each occurance of the "," character
  ...
  document.getElementById("deal").onclick = function () {
    //add a for loop that iterates through the contents of the cards node list. for each item do the following
    //if the length of the deck array is 0, call the newDeck() function
    for (let i = 0; i < cards.length; i++) {
      if (deck.length == 0) {
        newDeck();
      }
      //Use the pop() method to remive the last item from the deck array and store the popped item as the text content of the current item in the cards node list
      cards[i].textContent = deck.pop();
    }

    //change the valie of the tet ocntent of the cardsleft object to the value of the length of the deck array
    cardsLeft.textContent = deck.length;
  };
}
newDeck();
