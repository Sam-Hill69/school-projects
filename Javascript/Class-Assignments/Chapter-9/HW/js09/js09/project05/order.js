...
const CART_COUNT_KEY = "itemsInCart";

submitButton.onclick = function () {
  let itemTotal;
  // Increase the total items in the shopping cart by 1
  if (sessionStorage.getItem(CART_COUNT_KEY)) {
    itemTotal = parseInt(sessionStorage.getItem(CART_COUNT_KEY)) + 1;
  } else {
    itemTotal = 1;
  }

  // Store the number of items in the shopping cart
  sessionStorage.setItem(CART_COUNT_KEY, itemTotal);
