...
let phones = ["Photon 6E", "Photon 6X", "Photon 7E", "Photon 7X", "Photon 8P"]; // changed parenthese to square brackets

// Units sold in the previous quarter
let sales = [10281, 12255, 25718, 21403, 16142]; //changed parenthese to square brackets
...
sales.forEach(addToTotal); // changed addtoTotal to addToTotal
...
for (let i = 0; i <= phones.length; i++) {
  ...
  let barPercent = (sales[i] / totalSales) * 100; // added the index to sales for it to know which on to calculate
  ...
  switch (
    phones[i] // added colons to each case
  ) {
    case "Photon 6E":
  ...
  document
    ...
    .insertAdjacentHTML("beforeEnd", barChart); //added the 0 index  to tbody
}
