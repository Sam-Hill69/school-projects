...
function quoteGenerator() {
  ...
  let quoteCount = quotes.length; // added let

  // Generate a random integer to select a quote
  let randomQuote = randomInt(0, quoteCount); // added let

  // Retrieve a randomly-selected quote
  let quote = quotes[randomQuote]; // added let

  // Display the random quote
  document.getElementsByTagName("blockquote")[0].innerHTML = quote; // added an "s" to getElementsByTagName and changed the index to 0
}

/*=================================================================*/
// Function to return a randomly-selected integer between lowest and highest, inclusive
function randomInt(lowest, highest) {
  let size = highest - lowest; // removed the + 1
