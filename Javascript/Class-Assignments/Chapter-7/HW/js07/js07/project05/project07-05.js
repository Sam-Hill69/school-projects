...
document.getElementById("button1").onchange = function () {
  ...
  let file = this.files[0]; //changed file to files
...
function generateWordFreq(inputFile, outputDoc, outputCount) {
  ...
  let fr = new FileReader(); //added FileReader
  fr.read(inputFile);
  ...
  fr.onload = function () {
    ...
    let alphaRegx = /[^a-zA-Z\s]/g; //removed quotes and added a backslash to the s
